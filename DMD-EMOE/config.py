...
def get_config_regression(model_name, dataset_name, config_file=""):
    ...
    # ==================== 添加MoME相关配置 ====================
    if model_name.lower() == 'dmd':
        config['use_mome'] = True  # 启用MoME
        config['mome_fusion_type'] = 'sum'  # 融合类型：sum或concat
        config['mome_temperature'] = 0.5  # 路由器温度参数
        config['mome_alpha'] = 0.1  # 平衡损失权重
        config['mome_distill_weight'] = 0.3  # 蒸馏损失权重
